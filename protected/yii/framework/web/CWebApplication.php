...
class CWebApplication extends CApplication
{
	...
	{
		$this->_request_id = md5(uniqid(rand(0, 10000)));
		$this->_api_token = $this->request->getParam('token', 'unknown');
		return true;
	}
    ...
    /**
     * For alert log improvement.
     * Added some fields 'model_code', 'os_version', 'appli_version' and 'os_type'.
     */
	/**
	 * Set model_code(appli)
	 *
	 * @param string $model_code
	 */
	public function setModelCode($model_code = '')
	{
		$this->_model_code = $model_code;
	}
	/**
	 * Get model_code(appli)
	 *
	 * @return string
	 */
	...
	/**
	 * Set appli_version(appli)
	 *
	 * @param string $appli_version
	 */
	public function setAppliVersion($appli_version = '')
	{
		$this->_appli_version = $appli_version;
	}
	/**
	 * Get appli_version(appli)
	 *
	 * @return string
	 */
	public function getAppliVersion()
	{
		return $this->_appli_version;
	}
	/**
	 * Set OS type(appli)
	 *
	 * @param string $appli_id
	 */
	public function setOsType($appli_id = '')
	{
		if (substr($appli_id, 0, 1) === 'i') {
			$this->_os_type = 'iOS';
		} else if (substr($appli_id, 0, 1) === 'a') {
