			...
			if($isClass)
			{
				...
				else
                {
					if(is_file($path.'.php'))
                        ...
                        self::$classMap[$className]=$path.'.class.php';
					else
						throw new CException(Yii::t('yii','Alias "{alias}" is invalid. Make sure it points to an existing PHP file.',array('{alias}'=>$alias)));
                }
				return $className;
			...
			if(strpos($className,'\\')===false)  // class without namespace
			{
				if(self::$enableIncludePath===false)
				{
					foreach(self::$_includePaths as $path)
					{
						...
						$classFile=$path.DIRECTORY_SEPARATOR.$className.'.class.php';
						if(is_file($classFile))
						{
							include($classFile);
							break;
				...
				else
                {
                    ...
                    {
                        $classFile = $include_path . DIRECTORY_SEPARATOR . $className . '.php';
						if(is_file($classFile))
						{
							include($classFile);
							break;
						}
                        $classFile = $include_path . DIRECTORY_SEPARATOR . $className . '.class.php';
						if(is_file($classFile))
						{
							include($classFile);
							break;
